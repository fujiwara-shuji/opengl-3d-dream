...
struct RenderConfig
{
    ...
    float vertexDisplayRadius = 0.0133f;  // Visual radius of vertices when rendered (2/3 of 0.02)
    ...
    bool showEdges = false;         // Show/hide model edges (default: hidden for cleaner look)
...
{
    // Reflection control
    bool enableReflection = true;     // Enable/disable specular reflection system
    int maxReflectionDepth = 2;       // Maximum recursion depth for reflections
    ...
    Vector3 lightDirection = Vector3(0.3f, -0.3f, -1.0f).normalized(); // Light direction: slightly angled from above
    float ambientStrength = 0.8f;                                  // Ambient light strength (0.0 - 1.0)
    ...
    // Skybox / Sun settings
    bool enableSun = true;                               // Enable/disable sun disc rendering
    float sunAngularSize = 0.1f;                         // Angular size of sun (0.01 - 0.2, larger = bigger sun)
    Vector3 sunColor = Vector3(1.0f, 1.0f, 0.8f);        // Sun color (bright yellow-white)
    Vector3 skyHorizonColor = Vector3(0.6f, 0.8f, 1.0f); // Sky color near horizon (light blue)
    Vector3 skyZenithColor = Vector3(0.1f, 0.2f, 0.6f);  // Sky color at zenith (darker blue)
